...
function addNewAssetInteractive() returns error? {
    ...
    boolean success = check addAsset(newAsset);
    if success {
        io:println("Asset added successfully!");
    } else {
        io:println("Failed to add asset.");
    }
}
...
function viewAllAssets() returns error? {
    io:println("\n=== ALL ASSETS ===");
    Asset[]? assets = check getAllAssets();

    if assets is () || assets.length() == 0 {
...
function viewAssetByTag() returns error? {
    ...
    Asset? asset = check getAsset(assetTag);
    if asset is Asset {
...
function viewAssetsByFaculty() returns error? {
    ...
    Asset[]? assets = check getAssetsByFaculty(faculty);
...
function viewDueItems() returns error? {
    ...
    Asset[]? dueItems = check getDueItems();

    if dueItems is () || dueItems.length() == 0 {
...
function updateAssetInteractive() returns error? {
    ...
    Asset? currentAsset = check getAsset(assetTag);
    if currentAsset is () {
        io:println("Asset not found with tag: " + assetTag);
        return;
    }
    ...
    Asset updatedAsset = {
        ...
        name: name != "" ? name : currentAsset.name,
        faculty: faculty != "" ? faculty : currentAsset.faculty,
        department: department != "" ? department : currentAsset.department,
        dateAcquired: currentAsset.dateAcquired,
        current_status: status != "" ? status : currentAsset.current_status,
        components: currentAsset.components,
        schedules: currentAsset.schedules,
        workOrders: currentAsset.workOrders
    };

    boolean success = check updateAsset(updatedAsset);
    if success {
...
function deleteAssetInteractive() returns error? {
    ...
    if confirm.toLowerAscii() == "y" {
        boolean success = check deleteAsset(assetTag);
        if success {
...
function addAsset(Asset asset) returns boolean|error {
    http:Response res = check assetClient->post("/asset", asset);
    return res.statusCode == 201 || res.statusCode == 200;
}

function getAllAssets() returns Asset[]?|error {
    Asset[]|http:ClientError res = assetClient->get("/asset");
    if res is Asset[] {
        return res;
    }
    return ();
...
function getAsset(string assetTag) returns Asset?|error {
    Asset|http:ClientError res = assetClient->get("/asset/" + assetTag);
    if res is Asset {
        return res;
    }
    return ();
...
function getAssetsByFaculty(string faculty) returns Asset[]?|error {
    Asset[]|http:ClientError res = assetClient->get("/asset/faculty/" + faculty);
    if res is Asset[] {
        return res;
    }
    return ();
...
function getDueItems() returns Asset[]?|error {
    Asset[]|http:ClientError res = assetClient->get("/asset/due");
    if res is Asset[] {
        return res;
    }
    return ();
...
function updateAsset(Asset asset) returns boolean|error {
    http:Response res = check assetClient->put("/asset/" + asset.assetTag, asset);
    return res.statusCode == 200 || res.statusCode == 204;
}

function deleteAsset(string assetTag) returns boolean|error {
    http:Response res = check assetClient->delete("/asset/" + assetTag);
